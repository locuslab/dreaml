from hIndex import Index as Index
...
class TestIndex:
    ...
    def test_str(self):
        new_index = Index()
        keys = ["key" + str(s) for s in range(self.item_count)]
        vals = ["value" + str(s) for s in range(self.item_count)]
        new_index[keys] = vals
        print str(new_index)
    ...
    def test_key_exists(self):
        ...
        for k in keys:
            assert(k in new_index)
            assert(new_index.key_exists(k))

        assert(not new_index.key_exists("notakey"))
